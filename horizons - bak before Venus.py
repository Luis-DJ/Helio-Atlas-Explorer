...
def build_dataset() -> pd.DataFrame:
    ...
    venus_hc   = fetch_heliocentric_xyz_majorbody("299")
    ...
    df["venus_elong_deg"] = angle_deg_vectorized(-df["x_E"], -df["y_E"], -df["z_E"], df["x_V"] - df["x_E"], df["y_V"] - df["y_E"], df["z_V"] - df["z_E"])
    ...
    df["helio_sep_V_deg"] = angle_deg_vectorized(df["x_E"], df["y_E"], df["z_E"], df["x_V"], df["y_V"], df["z_V"])
