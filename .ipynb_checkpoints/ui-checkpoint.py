...
def update(
    ...
    # Markers
    V.set_offsets([[df["x_V"].iloc[i], df["y_V"].iloc[i]]])
    ...
    lineEV.set_data([df["x_E"].iloc[i], df["x_V"].iloc[i]], [df["y_E"].iloc[i], df["y_V"].iloc[i]])
    ...
    if SHOW_SOLAR_GLARE_MASK:
        ...
        style_connector(lineEV, float(df["venus_elong_deg"].iloc[i]))
